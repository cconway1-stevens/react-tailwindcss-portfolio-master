  ...
  useEffect(() => {
    ...
    async function step() {
      ...
      for (const i of currentOrder) {
        ...
        await gsap.delayedCall(2, handleDelayedCall, [
          slideItem,
          cardContent,
          ease,
        ]);
  ...
  return (
    <div className="relative">
      <div className="flex items-center justify-center min-h-screen bg-gray-100">
        <div
          id="demo"
          className="relative w-5/6 h-96 rounded-lg overflow-hidden bg-cover bg-center"
        ></div>
        <div
          id="slide-numbers"
          className="absolute bottom-0 right-0 mb-8 mr-8 grid grid-cols-1 gap-4"
        ></div>
      </div>
    </div>
  );
